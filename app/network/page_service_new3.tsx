      ...
      <div className="relative w-full flex-1 max-w-7xl mx-auto px-12 pb-16 overflow-hidden">
        <AnimatePresence mode="sync">
          <motion.div
            ...
            className={`absolute inset-x-12 top-0 bottom-4 rounded-[2rem] bg-gradient-to-b ${currentSection.bgGradient} flex flex-col p-8 shadow-[0_24px_60px_rgba(0,0,0,0.06)] border border-white/20 overflow-hidden`}
            ...
            <div className="relative z-10 flex-1 flex items-center justify-center w-full overflow-hidden">
              ...
              >
                ...
                {currentSection.id === "global" && currentSection.sectionHeading && (
                  <div className="flex flex-col justify-center text-left pr-6 overflow-hidden h-[440px]">
                ...
                {/* Main Cards Loop with 3D Flip Mechanics & Anti-Aliasing Fixes */}
                {currentSection.subContainers.map((card, cIdx) => (
                  <div
                    key={cIdx}
                    className="w-full h-[440px] perspective-[1200px] group select-none"
                  >
                    <div className="relative w-full h-full duration-700 transform-style-3d group-hover:rotate-y-180 transition-transform ease-[cubic-bezier(0.2,0.8,0.2,1)]">
                      ...
                      {/* --- FRONT SIDE --- */}
                      <div className="absolute inset-0 backface-hidden rounded-2xl bg-white border border-slate-100/80 flex flex-col p-5 justify-between shadow-[0_4px_20px_rgba(0,0,0,0.02)] overflow-hidden">
                        {/* 1. Card Title (Top Slot) */}
                        <div className="shrink-0 w-full mb-1">
                          <div className="flex items-start justify-between gap-2">
                            <h3 className="text-sm font-bold text-slate-900 group-hover:text-blue-600 transition-colors duration-300 leading-tight tracking-tight">
                              {card.title}
                            </h3>
                            <ArrowUpRight className="h-4 w-4 text-slate-400 group-hover:text-blue-600 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-300 shrink-0 mt-0.5" />
                          </div>
                        </div>

                        {/* 2. Image Graphic Box (Center Slot) */}
                        <div className="w-full h-28 rounded-xl bg-gradient-to-b from-[#f8fafc] to-[#f1f5f9] border border-slate-100/60 flex items-center justify-center relative overflow-hidden shadow-inner shrink-0">
                          {card.imageSrc && (
                            <img
                              src={card.imageSrc}
                              alt={card.title}
                        ...
                        {/* 3. Description Content Loop & Bullets */}
                        <div className="mt-2 flex-1 flex flex-col justify-start overflow-hidden w-full gap-2">
                          <p className="text-[10px] text-slate-500 leading-relaxed font-medium line-clamp-4 shrink-0">
                            {card.detail}
                          </p>
                          ...
                          {"features" in card && card.features && (
                            <div className="pt-2 border-t border-slate-100 shrink-0">
                              <span className="text-[9px] font-bold text-slate-800 tracking-wide uppercase block mb-1.5">
                                Key Features:
                              </span>
                              <div className="grid grid-cols-1 gap-0.5 text-[9px] font-medium text-slate-500">
                                {(card.features as string[]).map((feat, fIdx) => (
                                  <span key={fIdx} className="truncate flex items-center gap-1.5">
                                    <span className="text-blue-500 font-black">•</span> {feat}
                                  </span>
                                ))}
                              </div>
                            </div>
